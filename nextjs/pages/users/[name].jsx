        ...
        {user && repos && repos.map((repo) => (
          <div key={repo.id} className='repository-wrapper'>
            <a
              target='_blank'
              rel='noreferrer'
              href={`https://api.github.com/${user.login}/${repo.name}`}
            >
...
export const getServerSideProps = async ({ query }) => {
  ...
  try {
    ...
    const userRes = await fetch(`https://api.github.com/users/${name}`, {
      headers: {
        Authorization: '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'
      }
    })
    if (userRes.status === 200) {
      user = await userRes.json()
    }
    const repoRes = await fetch(`https://api.github.com/users/${name}/repos?sort=updated&page=1&per_page10`, {
      header: {
        Authorization: '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'
      }
    })
